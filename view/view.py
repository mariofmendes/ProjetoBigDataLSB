...
class VisualizacaoLSB:
    """Classe que renderiza os gráficos e tabelas no Streamlit"""

    def exibir_dashboard(self, df_filtrado, top_df, media_df, serie_df, metrica, ano_selecionado):
        # Título principal
        st.markdown(f"# Liga Super Basketball - Métrica: **{metrica}**")

        # Primeira linha com dois gráficos: top 10 e média por equipe
        col1, col2 = st.columns(2)

        # Gráfico de barras com top 10 equipes pela métrica
        ...
        # Gráfico de barras com média por equipe
        ...
        if ano_selecionado == "Todos" or ano_selecionado is None:
            ligas_no_ano = df_filtrado["Liga"].nunique()
            st.markdown(f"## 🔢 Contagem de ligas (Todos os anos): **{ligas_no_ano}**")
        else:
            ligas_no_ano = df_filtrado["Liga"].nunique()
            st.markdown(f"## 🔢 Contagem de ligas em {ano_selecionado}: **{ligas_no_ano}**")
